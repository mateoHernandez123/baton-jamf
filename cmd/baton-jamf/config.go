...
func validateConfig(ctx context.Context, cfg *config) error {
	if cfg.Username == "" {
		return fmt.Errorf("username is massing")
	}
	if cfg.Password == "" {
		return fmt.Errorf("password is massing")
	}
	if cfg.InstanceURL == "" {
		return fmt.Errorf("instanceurl is massing")
	}
...
func cmdFlags(cmd *cobra.Command){
	...
	cmd.PersistentFlags().String("instance-url", "", "URL of your Jamf Pro instance. ($BATON_INTANCE_URL)")
}
